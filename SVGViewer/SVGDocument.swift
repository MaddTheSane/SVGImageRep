...
class SVGView : NSView {
	...
	override func draw(_ dirtyRect: NSRect) {
		if let tempRef = NSGraphicsContext.current?.cgContext, let svg = renderContext {
			...
			tempRef.setFillColor(gray: 1, alpha: 1)
...
class SVGDocument: NSDocument {
	...
	@IBAction func reload(_ sender: AnyObject?) {
		...
		if let fileURL = fileURL {
			status = fileURL.withUnsafeFileSystemRepresentation({ (fsr) -> svg_status_t in
		...
		autoreleasepool {
			...
			status = svg_render(svg, &tmpEngine, UnsafeMutableRawPointer(Unmanaged.passUnretained(svg_render_context).toOpaque()))
